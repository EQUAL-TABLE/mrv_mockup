...
export const LCA_INTRO = {
  title: '커피의 탄소발자국, 「COFFEE MRV」로 계산합니다',
  paragraphs: [
    '전과정평가(LCA, Life Cycle Assessment)란 제품이 만들어져 쓰이고 버려지기까지 전 과정에서 나오는 환경 영향을 계산하는 방법입니다. 「COFFEE MRV」는 그 중 ‘탄소발자국’에 초점을 맞춰, 커피 한 제품이 남기는 온실가스 총량(탄소발자국)을 산정합니다. 커피는 「생두 재배 → 수송 → 로스팅 → 유통 → 사용 → 폐기」의 여러 단계를 거칩니다. 각 단계에서 발생하는 탄소를 모두 더해 ‘로스팅된 커피 1kg당 몇 kg의 CO₂가 나오는지’를 구하는 것이 커피 탄소회계입니다. ',
  ],
};
    ...
    options: [
      {
        ...
        tag: '탄소 회계 기반 산정',
        tagTone: 'primary',
        desc: '전기 고지서·거래명세서·INVOICE와 같은 실제 증빙 문서를 업로드하면 OCR로 값을 자동으로 읽어 계산합니다. 입력값이 증빙에 근거해 신뢰도가 높고, 검토·확정 절차를 거쳐 ISO 14067·환경성적표지 기준의 탄소 회계 산정 결과를 담은 결과확인서·보고서를 받아볼 수 있습니다.',
      },
      {
        name: '탄소배출량 추정치 계산기',
        tag: '참고용',
