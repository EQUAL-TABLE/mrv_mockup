...
export const LCA_INTRO = {
  title: '커피의 탄소발자국, 전과정평가(LCA)로 계산합니다',
  paragraphs: [
    '전과정평가(LCA, Life Cycle Assessment)는 제품이 만들어져 쓰이고 버려지기까지 전 과정에서 나오는 환경 영향을 계산하는 방법입니다. 이 시스템은 그중 ‘탄소’에 초점을 맞춰, 커피 한 제품이 남기는 온실가스 총량(탄소발자국)을 산정합니다.',
    '커피는 생두 재배 → 수송 → 로스팅 → 유통 → 사용 → 폐기의 여러 단계를 거칩니다. 각 단계에서 나오는 탄소를 모두 더해 ‘로스팅된 커피 1kg당 몇 kg의 CO₂가 나오는지’를 구하는 것이 커피 탄소회계입니다.',
    '정확한 산정을 위해 시작할 때 세 가지를 먼저 정합니다 — 어떻게 계산할지(방식), 어떤 표준을 따를지(방법론), 어디까지 계산할지(범위). 이 선택에 따라 진행 단계가 달라집니다.',
  ],
};
...
export const AXES: Axis[] = [
  {
    num: '1',
    title: '산정 방식 (트랙)',
    question: '증빙이 있나요, 대략만 볼까요?',
    options: [
      {
        name: 'MRV 기반 탄소배출량 산정',
        tag: '인증 가능',
        tagTone: 'primary',
        desc: '전기 고지서·거래명세서·인보이스 같은 실제 증빙을 올리면 시스템이 OCR로 값을 자동으로 읽어 계산합니다. 입력값이 증빙에 근거해 신뢰도가 높고, 검토·확정 절차를 거쳐 ISO 14067·환경성적표지 인증에 쓸 수 있는 결과확인서·보고서를 발급할 수 있습니다.',
      },
      {
        name: '탄소배출량 추정치 계산기',
        tag: '참고용',
  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        tag: '국내 인증',
        tagTone: 'primary',
        desc: '한국환경산업기술원의 환경성적표지 인증에 맞춘 국내 표준입니다. 범위는 ‘폐기까지’로 고정되며, 소비자 사용 단계 제외·전용 배출계수 적용 등 인증 고유의 규칙을 따릅니다. 국내 환경성적표지 인증 취득이 목표일 때 적합합니다.',
      },
    ],
  },
  {
